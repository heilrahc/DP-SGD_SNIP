...
class Pruner:
    ...
    def _global_mask(self, sparsity):
        ...
        # Threshold scores
        global_scores = torch.cat([torch.flatten(v)
    ...
    def _global_mask_str(self, sparsity):
        ...
        if not k < 1:
            threshold, _ = torch.kthvalue(
                channels_all, k)  # kth smallest value
            for i, (mask, param) in enumerate(self.masked_parameters):
                print(param.shape)
                score = self.scores[id(param)]
        ...
        for i, (mask, param) in enumerate(self.masked_parameters):
            ...
            if i != 0:  # inputs all but first
                if i == len(self.masked_parameters)-1:  # conv to fc
                    ...
                    if len(mask.shape) == 4 and not ((mask.shape[0] != mask.shape[1]) and mask.shape[2] == 1):
                        mask[:, removed[0]] = 0
                    else:
            ...
            if i != len(self.masked_parameters)-1:  # outputs all but last
                ...
                if len(param.shape) == 4:
                    if not ((mask.shape[0] != mask.shape[1]) and mask.shape[2] == 1):  # shortcut
                        s = score.mean(-1).mean(-1).mean(-1).flatten()
                if len(param.shape) == 2:
...
class SNIP(Pruner):
    ...
    def score(self, model, loss, dataloader, device):
        ...
        squared_param_norms = [
            m.batch_l2 for m, p in self.masked_parameters]
        # ...then compute the global norms...
        global_norms = torch.sqrt(
            torch.sum(torch.stack(squared_param_norms), dim=0))
        # ...and finally get a vector of clipping factors
        global_clips = torch.clamp_max(clip_norm / global_norms, 1.)

        # calculate score |g * theta|
        for m, p in self.masked_parameters:
            clipped_sample_grads = m.grad_batch * \
                expand_vector(global_clips, m.grad_batch)
            ...
            perturbed_grad = clipped_grad + \
                torch.randn_like(clipped_grad, device=device) * \
                noise_sdev  # ...and applied
        ...
        # normalize score
        all_scores = torch.cat([torch.flatten(v)
                               for v in self.scores.values()])
...
class GraSP(Pruner):
    ...
    def score(self, model, loss, dataloader, device):
        ...
        for batch_idx, (data, target) in enumerate(dataloader):
            ...
            flatten_grads = torch.cat([g.reshape(-1)
